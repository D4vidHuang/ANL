...
class OurAgent(DefaultParty):
    ...
    def __init__(self):
        ...
        self.perStart = 100
        self.perEnd = 150
        self.concessionCount = 0
        self.myRoundCount = 0
        self.stdUtil: float = -1
        self.meanUtil: float = -1
        self.all_received_bids = []
        self.recent_received_bids = []
        self.reservationBid = None
        self.reservationIndex = None
        self.acceptUti = 0.9
    ...
    def calculate_std(self, all_received_bids:List)->float:
        utilList = [self.profile.getUtility(b) for b in all_received_bids]
        mean = np.mean(utilList)
        std = np.std(utilList)
        return mean, std
    ...
    def opponent_action(self, action):
        ...
        if isinstance(action, Offer):
            ...
            self.all_received_bids.append(bid)
            if(len(self.all_received_bids) % 10 == 0):
                if(self.meanUtil < 0 and self.stdUtil < 0):
                    self.meanUtil, self.stdUtil = self.calculate_std(self.all_received_bids[-10:])
                else:
                    newMean, newStd = self.calculate_std(self.all_received_bids[-10:])
                    if(newStd > self.stdUtil):
                        if(self.stdUtil != 0):
                            inc_per = ((newStd - self.stdUtil)/self.stdUtil) * 100
                            if(inc_per > 30):
                                if(newMean > self.meanUtil):
                                    self.concessionCount += 1
                                else:
                                    # self.concessionCount = max(0, self.concessionCount - 1)
                                    self.concessionCount -= 1
                        else:
                            if (newMean > self.meanUtil):
                                self.concessionCount += 1
    ...
    def my_turn(self):
        ...
        if self.accept_condition(self.last_received_bid):
            ...
            if(self.concessionCount >= 3):
                self.perStart = max(self.perStart, self.perStart + 100)
                self.perEnd = max(self.perEnd, self.perEnd + 100)
                # self.concessionCount -= 5
                self.concessionCount = 0

            elif(self.concessionCount <= -3):
                self.perStart = max(0, self.perStart - 50)
                self.perEnd = max(15, self.perEnd -50)
                self.concessionCount = 0
            bid = self.find_bid()
                # bid = self.find_bid()
            # self.all_bids_list.append(bid)
    ...
    def accept_condition(self, bid: Bid) -> bool:
        ...
        # if progress > 0.95 and self.profile.getUtility(bid) > 0.6:
        if progress > 0.90:
            last_several_bids1 = self.all_received_bids[-1:5]
            last_several_bids2 = self.all_received_bids[-2:5]
            mean1, std1 = self.calculate_std(last_several_bids1)
            mean2, std2 = self.calculate_std(last_several_bids2)
            if(abs(std1 - std2) > 0.02):
                self.deadLineCount = max(0, self.deadLineCount + 1)
                self.isConcession = True
            else:
                self.deadLineCount = max(0, self.deadLineCount - 1)
            if(self.deadLineCount > 0):
                return False
            else:
                if(self.profile.getUtility(bid) > self.profile.getUtility(self.bids_with_utilities[self.reservationIndex][1])):
                    return True
                return False
        if(self.concessionCount >= 10):
            self.acceptUti = self.acceptUti * 0.95
        if(self.profile.getUtility(bid) >= self.acceptUti):
            return True
        return False
    ...
    def calculate_bids_with_utilities(self):
        ...
        print('bidlength:', self.all_bids.size())
        ...
        for i in range(len(self.bids_with_utilities)):
            if(self.bids_with_utilities[i][1] >= 0.5):
                self.reservationBid = self.bids_with_utilities[i][0]
                self.reservationIndex = i
    """
    ...
    def find_bid(self) -> Bid:
        ...
        if progress > 0.90 and hasattr(self, 'opponent_best_bid') and self.opponent_best_bid is not None :
            # return self.opponent_best_bid
            if (self.isConcession):
                rangeEnd = int(self.reservationIndex * 1.05)
                rangeStart = int(rangeEnd * 0.95)
                selectBidIndex = randint(rangeStart, rangeEnd + 1)

                bid = self.bids_with_utilities[selectBidIndex][0]
                self.reservationIndex -= 100
                self.isConcession = False
                return bid
            else:
                return self.opponent_best_bid
        ...
        # rangeStart = max(0, int(len(self.bids_with_utilities) * self.perStart))
        # rangeEnd = max(0, int(len(self.bids_with_utilities) * self.perEnd))
        rangeStart = max(0, self.perStart)
        rangeEnd = max(0, self.perEnd)
        selectBidIndex = randint(rangeStart, rangeEnd+1)

        if(self.bids_with_utilities[selectBidIndex][1] < 0.6):
            rangeEnd = self.reservationIndex
            rangeStart = int(rangeEnd * 0.95)
            selectBidIndex = randint(rangeStart, rangeEnd + 1)
